...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    while(!gpsQueue.empty())
    {
        ...
        printf("vio t: %f, gps t: %f delta_t: %f \n", t, gps_t, gps_t-t); 
        // 10ms sync tolerance
        //if(gps_t >= t - 0.01 && gps_t <= t + 0.01)
        if(gps_t >= t - 0.1 && gps_t <= t + 0.1) //TODO: get from config ( this can be larger for unsynced data)  //was 0.1 here
        {
            //printf("receive GPS with timestamp %f\n", GPS_msg->header.stamp.toSec());
            double latitude = GPS_msg->latitude;
            double longitude = GPS_msg->longitude;
            double altitude = GPS_msg->altitude;
            int fixstatus = GPS_msg->status.status;  //this is 2 for rtk 
            double pos_accuracy = GPS_msg->position_covariance[0]*1;
